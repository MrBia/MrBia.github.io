class TutorialPopup extends Popup {
    ...
    initElements() {
        ...
        this._title = this._scene.add.text(this._panel.position().x - 80, this._panel.position().y - this._panel.size().y * 0.37, 'Instructions', { fontFamily: 'molot', fontSize: 45, color: '#ff0000' });
        ...
        this._content1_1 = this._scene.add.text(this._panel.position().x - 165, this._panel.position().y - this._panel.size().y * 0.18, '       obtenir la boule.', { fontFamily: 'molot', fontSize: 30, color: '#61c200' });
